...
from .exact_ack_r3 import ExactAckSmacSession
...
class CorridorCache:
    ...
    def get(self,ctx,route,expansion=False):
        ...
        key=digest({'map':ctx.map_sha256,'shape':[m.height,m.width],'resolution':m.resolution,
                    ...
                    'bend':baseline.BEND_MARGIN_M,'algorithm':'adaptive-corridor-1-2-expand-2-4-r3-next3-v1'})
        if key in self.entries:
        ...
        mask,info=corridor(ctx,route,expansion=expansion);packed=np.packbits(mask)
...
def corridor(ctx,route,expansion=False):
    ...
    radius=math.ceil(((2. if expansion else 1.)+baseline.FOOTPRINT_SAFETY_MARGIN_M+baseline.BEND_MARGIN_M)/m.resolution)
    ...
    for i in range(1,len(points)-1):
        ...
        clearance=float(m.clearance(*points[i]) or 0.)
        widening=(4. if expansion else 2.) + max(0.,.5-clearance)
...
def run_query(ctx,query,topology,selector,session,spec,auditor,request_id,budget_s=7.,*,preparer=None):
    ...
    if route is None:
        ...
        for attempt in range(2):
            ...
            mb=time.monotonic();mask,maskinfo=session._corridor_cache.get(ctx,route,expansion=bool(attempt))
...
def main(argv=None):
    ...
    args=p.parse_args(argv);initialization_begin=time.monotonic();initialization={}
    ...
    protocol={'architecture_id':'2A-V1','implementation_revision':REVISION,'protocol_id':PROTOCOL_ID,
              'map_preparation':args.map_preparation,
    ...
    protocol['runtime_caches']={'corridor_packed_max_bytes':128*1024**2,'expected_effective_max_bytes':512*1024**2,'query_topology_capacity':32}
    protocol['server_budget_policy']={'maximum_s':5.,'reconfiguration_reserve_s':1.25,'quantum_s':.5,
                                      'upgrade_policy':'upgrade_gain_exceeds_rebuild_reserve_v1',
    ...
    preparer=QueryTopologyPreparer(tile,legacy.FOOTPRINT);prep=[]
    ...
    if args.stage=='offline':
        ...
        return 0
    auditor=PathAuditor(ctx,source_commit=REVISION);spec=legacy.backend_availability()['hybrid_astar']
    if not spec.available:raise RuntimeError(spec.reason)
    session=ExactAckSmacSession(ctx,output,map_yaml=ctx.map_yaml,log_tag='r3_'+args.map_id,
            ...
            for mode,count in loops:
                for repetition in range(1,count+1):
                    for query in queries:
                        ...
                        if session._unresolved_timeout or session._budget_state_uncertain:
