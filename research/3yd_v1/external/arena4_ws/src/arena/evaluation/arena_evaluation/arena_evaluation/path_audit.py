...
class PathAuditor:
    ...
    def __init__(self, ctx: Any, *, source_commit: str, footprint: Sequence[Sequence[float]] = legacy.FOOTPRINT):
        ...
        if getattr(ctx, "bounded_memory", False):
            from .bounded_context_r3 import BoundedField
            threshold = max(math.hypot(*v) for v in self.footprint) + math.sqrt(2.) * ctx.hospital_map.resolution / 2.
            self._distance_to_unsafe_m = BoundedField(ctx.hospital_map, 'audit', safe_threshold=threshold)
        else:
    ...
    def audit(
        ...
        clearance_started_ns = time.monotonic_ns()
        if hasattr(hospital_map.distance_m, "minimum_at"):
            minimum_clearance = hospital_map.distance_m.minimum_at([
                hospital_map.world_to_cell(float(point["x"]), float(point["y"])) for point in points])
        else:
            minimum_clearance = min((hospital_map.clearance(float(point["x"]), float(point["y"])) or 0.0) for point in points)
        timings["minimum_clearance_ms"] = (time.monotonic_ns() - clearance_started_ns) / 1.0e6
        metrics = {
